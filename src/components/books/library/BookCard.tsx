...
export default function BookCard({ book, view }: BookCardProps) {
  ...
  if (view === "list") {
    return (
      <button
        ...
        className="flex items-center gap-4 p-3 rounded-lg border border-border bg-card hover:bg-accent/50 transition-colors text-left w-full"
      >
  ...
  return (
    <button
      onClick={() => navigate(`/books/read/${book.id}`)}
      className="group flex flex-col rounded-xl border border-border bg-card overflow-hidden hover:shadow-lg transition-all hover:-translate-y-0.5"
    >
