...
import GiftAlertFields from "./GiftAlertFields";
import AlertTitleInput from "./AlertTitleInput";
import AlertSubmitButton from "./AlertSubmitButton";
...
const AlertDialogForm = ({ 
  ...
  // Alert type states
  const [isGiftAlert, setIsGiftAlert] = useState(initialType === 'gift');
  const [isMessageAlert, setIsMessageAlert] = useState(initialType === 'message');
    ...
    try {
      if (isMessageAlert) {
        if (isTemplate) {
          ...
          // For regular alerts, save to alerts table
          const alertData = {
            title,
            media_type: 'message',
            media_url: 'none',
